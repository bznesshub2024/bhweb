...
$datetime = date('Y-m-d H:i:s'); 
 ...
 // echo " dashboard redirect to index";
	...
	if($code==$_SESSION['_token']){
		...
		$prod_name_ar = trim(strip_tags($_POST['prod_name']));
		$prod_short_ar = trim(strip_tags($_POST['prod_short']));
		$prod_details_ar = trim(strip_tags($_POST['prod_details']));
		...
		if(isset($prod_name)  && isset($category) ) {
			...
			if($check_exist ==0){
				...
				$stmt11 = $conn->prepare("INSERT INTO `product_details`(`status`, `prod_name`, `prod_desc`, `prod_fulldetail`,
							...
							`product_manuf_country`, `product_hsn_code`, `product_video_url`, `return_policy_id`,`product_unique_id`,`featured_img`,created_at,created_by,regarding_price,
							prod_name_ar,prod_desc_ar,prod_fulldetail_ar,shipping,prod_weight,product_unique_code) 
							VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				
				$stmt11->bind_param( "issssiiiisssississssissssss",$enableproduct,$prod_name,$prod_short,$prod_details,$prod_img_url,$selectattrset,$selectbrand,$prod_type,$price_type,
					$prod_url,$prod_sku,$selectvisibility,$selectcountry,$prod_hsn,$prod_youtubeid,$return_policy,$product_unique_id,$featured_img,$datetime,$_SESSION['admin'],$regarding_price,
					$prod_name_ar,$prod_short_ar,$prod_details_ar,$shipping,$prod_weight,$product_unique_code);
				...
				if($rows>0){
					...
					$vendor_prod_sql = $conn->prepare("INSERT INTO `vendor_product`( `product_id`, `vendor_id`, `product_mrp`, `product_sale_price`, `product_tax_class`, 
										`product_stock`, `stock_status`, `product_purchase_limit`, `product_remark`, `product_related_prod`, `product_upsell_prod`,`enable_status`,`created_at`,`seller_price`) 
										VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					
					$vendor_prod_sql->bind_param( "sssssisissssss",$prod_id,$selectseller,$prod_mrp,$prod_price,$selecttaxclass,$prod_qty,$selectstock,
							$prod_purchase_lmt,$prod_remark,$selectrelatedprod,$selectupsell,$enableproduct,$datetime,$seller_price);
