...
class Product extends REST_Controller {
	...
	public function getProductDetails_post(){
    	...
    	if($validation=='valid') {			
			$product_custom_cloth = $this->product_model->get_product_custom_cloth($pid);
      		$product_array = $this->product_model->get_product_request($language_code,$pid,$sku,$sid,$devicetype);
	...
	public function getProductDetails_seller_post(){
		$requiredparameters = array('language','pid','sku','sid','web_url','devicetype');
    	...
    	if($validation=='valid') {			
			$product_custom_cloth = $this->product_model->get_product_custom_cloth($pid);
      		$product_array = $this->product_model->get_product_seller_request($language_code,$pid,$sku,$sid,$devicetype);
	...
	function getProductFilter_post(){
    	...
    	if($validation=='valid') {
			if($cat_id){
				...
				$main_cat1 = $this->product_model->get_sub_category_request($cat_id);
				if(!empty($main_cat1))
				{
					$main_cat2 = $this->product_model->get_sub_category_request($main_cat1);
				}
				else
				{
					$main_cat2 = array();
				...
				if(!empty($main_cat))
				{
					$sort_array = $this->product_model->get_product_filter($main_cat);
				}	
				else
				{
