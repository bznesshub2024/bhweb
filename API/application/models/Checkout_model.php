...
class Checkout_model extends CI_Model {
	...
	function get_checkout_full_details($language, $user_id,$quote_id,$shipping_city='',$coupon_code = '',$shipping_pincode = '', $payment_method = ''){
		...
		if($query->num_rows() >0){
			...
			foreach($cart_result as $cart_detail){
				...
				$this->db->select("product_sku, prod_name, prod_name_ar,featured_img,web_url,is_heavy,sellerlogin.companyname as seller, vp.product_mrp, vp.product_sale_price, vp.product_stock, vp.product_purchase_limit, vp.id as vendor_prod_id,vp.coupon_code,vp.seller_price,vp.product_tax_class,sellerlogin.pincode as seller_pincode");
				...
				if($query_prod->num_rows() >0){
					...
					$net_tax_amount = ($product_detail['seller_price'] * 100)/ ($product_detail['coupon_tax'] + 100);
				...
				$shipping_msg = '';
				if ($shipping_pincode && $payment_method) {
					$shipping_data = $this->calculateShippingFee($product_detail['seller_pincode'], $shipping_pincode, '10', '10','10', '300', $payment_method, $product_detail['price1'] / intval($product_detail['qty']),intval($product_detail['qty']));
					$shipping_data_array[] = array(
						'prod_id' => $product_detail['prodid'],
						'prod_qty' => intval($product_detail['qty']),
						'shipping_data' => $shipping_data
					);

					if ($shipping_data['status']) {
						// $shipping = round(floatval($shipping_data['data']['min_delivery_price']) * intval($product_detail['qty']));
						$shipping = round($shipping_data['data']['min_delivery_price']);
						$total_shipping_fee += $shipping;
						$shipping_msg = 'Get it by '.$shipping_data['data']['delivery_day'];
					} else {
						$shipping = 0;
						$total_shipping_fee += $shipping;
						$shipping_msg = '';
		...
		if($total_item >0){
			return array('user_address' =>$user_address,'Products'=>$product_detail_array, 'total_mrp' =>price_format($total_mrp), 'total_discount' =>price_format($total_mrp-$total_price),'total_price'=>price_format($total_price),
				'total_item'=>$total_item, 'tax_payable'=>number_format($tax_amount), 'coupon_code'=>$coupon_code, 'coupon_discount'=>number_format($coupon_discount),'shipping_msg' => $shipping_msg, 'shipping_fee'=>price_format($shipping_fee), 'payable_amount'=>price_format($total_price+$shipping_fee-$coupon_discount-$default_discount),'payable_amount_value'=>number_format($total_price+$shipping_fee-$coupon_discount-$default_discount,2, '.', ''),'total_price_value'=>number_format($total_price+$shipping_fee-$coupon_discount-$default_discount,2, '.', ''),'default_discount' => round($default_discount,0));
		}else{
			return false;
		}
	}
	...
	function place_order_details($language, $user_id,$qouteid,$fullname,$mobile,$locality,$fulladdress,$city,$state,$pincode,$addresstype,$email,$payment_id,$payment_mode,$coupon_code,$city_id,$coupon_value,$lat,$lang){
		$status =array('status'=>'');
		...
		if($query->num_rows() >0){
			...
			$add_order = $this->create_order($order_id,$user_id,$qouteid,$fullname,$mobile,$locality,$fulladdress,$city,$state,$pincode,$addresstype,$email,$payment_id,$payment_mode,$coupon_code,$city_id,$coupon_value,$lat,$lang);
			...
			foreach($cart_result as $cart_detail){
				...
				$this->db->select("product_sku, prod_name, prod_name_ar,featured_img,web_url,is_heavy,return_policy_id, vp.product_mrp, vp.product_sale_price, vp.product_stock, vp.product_purchase_limit, vp.id as vendor_prod_id,
									prp.policy_validity,prp.policy_type_refund,prp.policy_type_replace,prp.policy_type_exchange,vp.seller_price,vp.product_tax_class,product_hsn_code,product_unique_code,sellerlogin.pincode as seller_pincode,sellerlogin.phone as seller_phone");
				...
				if($query_prod->num_rows() >0){
					...
					curl_setopt($curl_handle, CURLOPT_URL, 'https://api.postalpincode.in/pincode/' .$prod_result[0]->seller_pincode);
					...
					$order_prod['shipping'] = $shipping_fee1;
					...
					$order_prod['seller_price'] = $prod_result[0]->seller_price;
					...
					$order_prod['product_hsn_code'] = $prod_result[0]->product_hsn_code;
					$order_prod['product_unique_code'] = $prod_result[0]->product_unique_code;
				...
				$data_wallet_history['remark'] = 'Admin Order Commison From '.$fullname; 
				...
				if($level_1_user_id != admin_user_id && $level_1_user_id != '')
				{
					...
					$data_wallet_history['order_id'] = $order_id;
					$data_wallet_history['user_id'] = $user_id;
					$data_wallet_history['remark'] = 'Order Commison From referr '.$fullname; 
					$data_wallet_history['created_at'] = $this->date_time;
					$this->db->insert('wallet_transaction_history',$data_wallet_history);
	...
	function create_order($order_id,$user_id,$qouteid,$fullname,$mobile,$locality,$fulladdress,$city,$state,$pincode,$addresstype,$email,$payment_id,$payment_mode,$coupon_code,$city_id,$coupon_value,$lat,$lang){
		$status = '';
		$order = array();
