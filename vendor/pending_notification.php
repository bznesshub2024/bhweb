    	   ...
    	   $stmt->execute();	 
			...
			$category_count = 0;
       	   while ($stmt->fetch()) { 
				if($col1 >0){
					$category_count++;
					$html .= '<li><p><a href="javascript:void(0)" onclick="page_redirect(\'pending_category.php\')">'.$col2.'</a></p></li>';
				}
    	    }
			
			
			 $stmt1 = $conn->prepare("SELECT count(id) FROM order_product where vendor_id ='".$_SESSION['admin']."' AND status ='Return Requested' order by id desc");
    	   
    	   $stmt1->execute();	 
     	   $data = $stmt1->bind_result( $col10);
          
			$order_count = 0;
       	   while ($stmt1->fetch()) { 
				if($col10 >0){
					$order_count++;
					$html .= '<li><p><a href="javascript:void(0)" onclick="page_redirect(\'manage_orders.php\')">Pending Returned Order ('.$col10.')</a></p></li>';
				}
    	    }
    	    ...
    	    $total_count = $category_count+$order_count;
