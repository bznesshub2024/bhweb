...
if (!isset($_SESSION['admin'])) {
   ...
   try{
		...
		if(isset($_POST['prod_name'])){
			...
			$query = "SELECT pd.product_unique_id, pd.prod_name, pd.featured_img,  vp.enable_status ,brand.brand_name,pd.product_sku  FROM product_details pd,brand,product_category, vendor_product vp WHERE vp.product_id =pd.product_unique_id AND vp.vendor_id ='".$selectseller."'   AND brand.brand_id  = pd.brand_id and product_category.prod_id = pd.product_unique_id AND vp.enable_status IN(1,3)  AND (pd.product_unique_id like '%".$search_text."%'  OR pd.prod_name like '%".$search_text."%'  OR pd.product_sku like '%".$search_text."%'  ) ".$cat_data."
				".$order_by." LIMIT ?, ?";
			
			$query_total = "SELECT count(pd.product_unique_id) FROM product_details pd,brand ,product_category, vendor_product vp WHERE vp.product_id =pd.product_unique_id AND vp.vendor_id ='".$selectseller."'   AND brand.brand_id  = pd.brand_id and product_category.prod_id = pd.product_unique_id AND vp.enable_status IN(1,3) ".$cat_data."   AND (pd.product_unique_id like '%".$search_text."%'  OR pd.prod_name like '%".$search_text."%'  OR pd.product_sku like '%".$search_text."%'  )";
		}else{
			$query = "SELECT pd.product_unique_id, pd.prod_name, pd.featured_img,  vp.enable_status ,brand.brand_name,pd.product_sku
				FROM product_details pd,brand ,product_category, vendor_product vp WHERE vp.product_id =pd.product_unique_id AND vp.vendor_id ='".$selectseller."'   AND brand.brand_id  = pd.brand_id and product_category.prod_id = pd.product_unique_id AND vp.enable_status IN(1,3) ".$cat_data."    ".$order_by." LIMIT ?, ?";
				
			$query_total = "SELECT count(pd.product_unique_id) FROM product_details pd,brand ,product_category, vendor_product vp WHERE vp.product_id =pd.product_unique_id AND vp.vendor_id ='".$selectseller."' and product_category.prod_id = pd.product_unique_id  AND brand.brand_id  = pd.brand_id AND vp.enable_status IN(1,3) ".$cat_data."  ";
		}
		...
		foreach($product_arr_final as $products){
            ...
            $tbl_html .=  '<td>
								<div class="dropdown">
									...
									<ul class="dropdown-menu" style="width:104px;">
										<li><a href="view_product.php?id='.$products['product_unique_id'].'"><i class="fa fa-eye"></i> View</a></li>
										<li><a href="edit_product.php?id='.$products['product_unique_id'].'"><i class="fa fa-edit"></i> Edit</a></li>
										<li><a href="javascript:void(0)" onclick=delete_products("'.$products['product_unique_id'].'")><i class="fa fa-trash"></i> Delete</a></li>
									</ul>
								</div>
							</td></tr>';
           $i++;
