...
class Common_Function {
	...
	}	function file_upload_video($file_name,$media_path){		include_once('libraries/php-image-resize-master/lib/ImageResize.php');				$media_dir = $this->create_media_folder($media_path);		$file_full_path_final = '';		if($_FILES[$file_name]["name"]!=""){				$path_info = pathinfo($_FILES[$file_name]["name"]);								$extension = $path_info['extension'];				$filename = $this->makeimagepath($path_info['filename']);								$intFile = mt_rand();				$file_full_path1 = $media_dir.'/'.$filename.$intFile.'.'.$extension;				move_uploaded_file($_FILES[$file_name]["tmp_name"],$file_full_path1);								$destination_file = $media_dir.'/'.$filename.$intFile.'.'.$extension;					$file_full_path = str_replace($media_path,'',$destination_file);				$file_full_path_final = str_replace('"','',$file_full_path);			}		return $file_full_path_final;	}
	...
	function finish_import_bulk_product($conn,$rows,$seller,$datetime,$table_name){
		...
		if($stmt_check1->num_rows > 0){
			...
			$stmt11->bind_param( "issssiiiisssississss",$enableproduct,$prod_name,$prod_short,$prod_details,$prod_img_url,$selectattrset,$selectbrand,$prod_type,$price_type,
				$prod_url,$prod_sku,$selectvisibility,$selectcountry,$prod_hsn,$prod_youtubeid,$return_policy,$product_unique_id,$featured_img,$datetime,$seller);
	...
	function generate_invoice($conn,$ordersno,$product_id,$save_pdf){
	  ...
	  $stmt_tax = $conn->prepare("SELECT  percent FROM tax WHERE tax_id=?");
		...
		$cname="";  $address_seller=""; $tax=""; $pan_number= $cin_number = ""; 
	   ...
	   $stmt = $conn->prepare("SELECT  companyname, fullname, address, city, pincode, state, country, phone, email, logo,
		...
		//echo " get col data ";
		while ($stmt->fetch()) {           
              $cname=$col1; $fname=$col2; $address_seller=$col3;  $city=$col4;
              $pincode =$col5; $state=$col6; $country=$col7; $phone=$col8; 
              $email=$col9; $logo=$col10; $tax=$col11;   $pan_number= $col12; $cin_number =$col13;       
	...
	$html2 = '
		<html>
			...
			<body>
				...
				<table style="line-height: 1.5;font-size:9px;">
					<tr><td ><b>Sold By: <span>'.$cname.'</span></b> </td>
						<td style="text-align:right;"><b>Invoice Number:</b> # '.$invoice_number.'</td>
					</tr>
					<tr>
						<td><b>Ship-from Address:</b> <span>'.$address_seller.'</span></td>
					</tr>
					<tr>
						<td  colspan="2"><b>GSTIN: </b>'.$tax.'</td>
				...
				<table style="line-height: 1.5;font-size:9px;">
					<tr>
						<td width="35%"><b>Order ID:</b> <span>'.$ordersno.' </span></td>
						<td width="25%" style="text-align:left;"><b>Bill To</b></td>
						<td width="25%" style="text-align:left;"><b>Shipping To</b></td>
						<td width="15%"style="text-align:left;"></td>
					</tr>
					<tr>
						<td><b>Order Date:</b> <span>'.$create_date.'</span></td>
						<td style="text-align:left;"><b>'.$fullname.'</b></td>
						<td style="text-align:left;"><b>'.$fullname.'</b></td>
						<td style="text-align:left;" rowspan="5">*Keep this invoice and manufacturer box for warranty purposes.</td>
					</tr>
					<tr>
						<td><b>Invoice Date:</b> <span> '.$create_date.'</span></td>
						<td style="text-align:left;"><span>'.$address.'</span></td>
						<td style="text-align:left;"><span>'.$address.'</span></td>
					</tr>
					<tr>
						<td><b></b> <span></span></td>
						<td style="text-align:left;"><b>Phone:</b> <span> '.$mobile.'</span></td>
						<td style="text-align:left;"><b>Phone:</b> <span> '.$mobile.'</span></td>
					</tr>
					<tr>
						<td><b></b> <span></span></td>
				...
				<div style="border-bottom:1px solid #000;  margin-left: 10%;">
					<table style="line-height: 2;font-size:9px;">
						<tr style="font-weight: bold;border:1px solid #cccccc;background-color:#f2f2f2;">
							<td style="border:1px solid #cccccc;width:120px;">Item Description</td>
							<td style = "text-align:right;border:1px solid #cccccc;width:45px">Price ('.$currency.')</td>
							<td style = "text-align:right;border:1px solid #cccccc;width:35px;">Discount</td>
							...
							<td style = "text-align:right;border:1px solid #cccccc;width:38px;">CGST</td>
							<td style = "text-align:right;border:1px solid #cccccc;width:38px;">SGST</td>
							<td style = "text-align:right;border:1px solid #cccccc;width:38px;">IGST</td>
							<td style = "text-align:right;border:1px solid #cccccc;width:48px;">Subtotal ('.$currency.')</td>
						</tr>
						<tr>
							<td style="border:1px solid #cccccc;">'.$prod_name.'<br>HSN Code - '.$product_hsn_code.'<br>'.$prod_attr1.'</td>
							...
							<td style = "text-align:right; border:1px solid #cccccc;">'.$cgst.'</td>
							<td style = "text-align:right; border:1px solid #cccccc;">'.$sgst.'</td>
						...
						<tr style = "font-weight: bold;">
							...
							<td style = "text-align:right;width:100px">Total ('.$currency.')</td>
							<td style = "text-align:right;width:140px">'.$subtotal+$shipping.'</td>
						</tr>
						<tr><td></td></tr>
						<tr>
							<td></td><td></td><td></td><td></td>
							<td style = "text-align:right;"></td>
							<td style = "text-align:right;width:140px">Authorized Signatory</td>
						</tr>
					</table>
				</div>
