...
if($code==$_SESSION['_token'] && isset($coupancode)   && !empty($coupancode)&& isset($cvalue)   && !empty($cvalue)  ) {
       ...
       $active ="active";
        $stmt11 = $conn->prepare("INSERT INTO coupancode_vendor( name, value, fromdate, todate, activate, cap_value, min_order,coupon_type,user_apply,vendor_id,coupandesc )  VALUES (?,?,?,?,?,?,?,?,?,?,?)");
		$stmt11->bind_param( 'sisssiiiiss',  $coupancode , $cvalue, $fromdate,$todate, $active, $capvalue, $minorder,$coupon_type,$user_apply,$vendor_id,$coupandesc);
